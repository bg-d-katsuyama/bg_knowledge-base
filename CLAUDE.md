...
BG社のナレッジ（ミーティングメモ、Google Meet議事録、Drive文書、Slackメッセージ）を一元化し、構造化検索可能なナレッジベースとしてNotion上に構築する社内システム。詳細は `docs/architecture.md` を必ず参照してください。

**現在の運用方式（2026-05-29 改訂・ローカル完結方式）**: Claude Code が `data/drive_input/` の議事録（docx）から土壌R&Dの知見を抽出し、`output/` にマスターデータ xlsx を生成します。GCP / Anthropic API / Drive API / Sheets API は使用しません（外部 API 課金ゼロ）。Notion は読み取り専用です。日常の運用手順は `docs/runbook.md` を参照してください。

**主たる利用者**:
- 久保田様（運用責任者。Claude Code で日常運用）
- スタッフ（Notion・マスターデータのスプレッドシートを参照）
- 勝山様（開発・保守。移行期間中は運用も併走）

**不変条件（必ず守る）**:
- BGワークスペースの既存 Notion ページ・DB（MTG Minutes DB などのソース）は**絶対に編集・削除しない**（2026-05-08 に不変条件として確定。`docs/decision_log.md` 参照）
- 現行のローカル完結運用では、本システムが構築した KB 5 DB を含め **Notion への書き込みは一切行わない**。書き込みが必要になった場合は実行前に勝山様・久保田様の確認を取る
...
Phase 2 は 2026-05-29 に「土壌R&Dマスターデータ作成（ローカル完結方式）」へ全面改訂され、GCP 連携・定期同期は廃止されました。Phase 3・4 は保留中です。

| Phase | 内容 | 状態 |
|---|---|---|
| 0 | 環境構築（Notionスキーマ5DB構築） | 完了（GCP 関連は不要化） |
| 1 | Notion内完結（既存メモ・Slack の取り込み、マスタDBマージ） | 完了（検証済み） |
| 2 | 土壌R&Dマスターデータ作成（ローカル完結方式） | 進行中＋運用移管（移行期間） |
| 3 | Slack連携（Bot、`/kb-*` コマンド） | 保留 |
| 4 | 運用定着（検索強化・ダッシュボード） | 保留 |
...
- 運用責任者: 久保田様
- 2026-06 より移行期間として勝山様・久保田様の両名が運用操作可能（併走ルールは `docs/runbook.md` 第2章を参照）
